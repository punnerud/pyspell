...
// PySpell add-on: serves a web text window + `/run` API over the Tailscale
// tunnel. The ONLY ESP/PySpell-aware glue lives here; tailscale-core just holds
...
#[cfg(feature = "http-server")]
pub(crate) fn new_tcp_server() -> tailscale_core::tcp::TcpServer {
    #[cfg(feature = "pyspell")]
    {
        tailscale_core::tcp::TcpServer::with_handler(pyspell_web::route)
    }
    #[cfg(not(feature = "pyspell"))]
    {
        tailscale_core::tcp::TcpServer::new()
    }
}
...
fn main() -> Result<()> {
    ...
    // --- APA102 RGB LED on the underside (GPIO40 = data, GPIO39 = clock) ---
    // The T-Dongle S3 has an addressable APA102 LED that powers up lit in an
    // undefined state. We turn it off at boot (it latches), and keep the pins so
    // the UI thread can toggle it from the in-tunnel web panel later.
    let mut led_data = PinDriver::output(pins.gpio40)?;
    let mut led_clk = PinDriver::output(pins.gpio39)?;
    {
        // APA102 frame: start (4×0x00) | LED [0xE0|brightness, B, G, R] | end (4×0xFF)
        let mut frame = |bri: u8, b: u8, g: u8, r: u8| {
            for x in [0x00, 0x00, 0x00, 0x00, 0xE0 | (bri & 0x1F), b, g, r,
                      0xFF, 0xFF, 0xFF, 0xFF] {
                for i in (0..8).rev() {
                    if (x >> i) & 1 == 1 {
                        let _ = led_data.set_high();
                    } else {
                        let _ = led_data.set_low();
                    }
                    let _ = led_clk.set_high();
                    let _ = led_clk.set_low();
                }
            }
        };
        frame(0, 0, 0, 0); // off
    ...
    // Leak a 'static SPI scratch buffer so the resulting `display` is 'static and
    // can be moved into the UI thread (which owns it for the device's lifetime).
    ...
    // WiFi power-save (modem sleep) per config::WIFI_POWER_SAVE. `None` keeps the
    // radio always on (lowest latency); `Min`/`Max` save power at the cost of
    // ~100-300ms extra incoming-packet latency (inflates tunnel ping/HTTP).
    let ps_mode = match config::WIFI_POWER_SAVE {
        config::WifiPowerSave::None => esp_idf_svc::sys::wifi_ps_type_t_WIFI_PS_NONE,
        config::WifiPowerSave::Min => esp_idf_svc::sys::wifi_ps_type_t_WIFI_PS_MIN_MODEM,
        config::WifiPowerSave::Max => esp_idf_svc::sys::wifi_ps_type_t_WIFI_PS_MAX_MODEM,
    };
    unsafe {
    ...
    if status_line == "registered OK" {
        ...
        // --- UI control thread: from here on it owns the display, backlight and
        // LED. It draws the default Tailscale-info screen and reacts to commands
        // from the in-tunnel web panel (LED, backlight, flash, text) and to
        // status updates we post from the map-stream loop below. ---
        let (ui_tx, ui_rx) = std::sync::mpsc::channel::<ui::UiCommand>();
        ui::set_sender(ui_tx);
        {
            let lan0 = lan_line.clone();
            let mut display = display;
            let mut backlight = backlight;
            let mut led_data = led_data;
            let mut led_clk = led_clk;
            let _ = std::thread::Builder::new()
                .name("ui".into())
                .stack_size(8 * 1024)
                .spawn(move || {
                    ...
                    use ui::UiCommand;
                    // APA102 LED writer (bit-bang), capturing the two LED pins.
                    let mut led = |bri: u8, b: u8, g: u8, r: u8| {
                        for x in [0u8, 0, 0, 0, 0xE0 | (bri & 0x1F), b, g, r, 0xFF, 0xFF, 0xFF, 0xFF]
                        {
                            for i in (0..8).rev() {
                                if (x >> i) & 1 == 1 {
                                    let _ = led_data.set_high();
...
/// (char-based, honouring explicit newlines). Used for the web-panel "Vis tekst".
fn wrap_text(s: &str, width: usize, max_lines: usize) -> Vec<String> {
    let mut lines: Vec<String> = Vec::new();
    let mut cur = String::new();
    for ch in s.chars() {
        if ch == '\n' || cur.chars().count() >= width {
